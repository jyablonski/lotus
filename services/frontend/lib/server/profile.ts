...
function calculateBasicCounts(journals: JournalEntry[]) {
  const now = new Date();
...
function calculateAverageMood(journals: JournalEntry[]): number {
  if (!journals.length) return 0;
  return journals.reduce((sum, j) => sum + j.userMood, 0) / journals.length;
}

function getFirstEntryDate(journals: JournalEntry[]): string | null {
  if (!journals.length) return null;
  return journals[journals.length - 1].createdAt;
}

function getMostActiveDay(journals: JournalEntry[]): string {
...
function getFavoriteMoodCategory(journals: JournalEntry[]): string {
  if (!journals.length) return "No data";
...
function calculateTotalWords(journals: JournalEntry[]): number {
...
function calculateProfileStats(journals: JournalEntry[]): ProfileStats {
  if (!journals.length) {
    return {
      totalEntries: 0,
      thisMonth: 0,
 ...
 * Fetches journals and calculates all statistics
 */
