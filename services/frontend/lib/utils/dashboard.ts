...
export function calculateAverageMood(journals: JournalEntry[], days: number = 7): number {
    const now = new Date();
    const daysAgo = new Date(now.getTime() - days * 24 * 60 * 60 * 1000);

    const recentEntries = journals.filter(journal =>
        new Date(journal.createdAt) >= daysAgo
    );

    if (recentEntries.length === 0) return 0;

    return recentEntries.reduce((sum, entry) => sum + parseInt(entry.userMood), 0) / recentEntries.length;
}
...
export function getSentimentFromMoodInt(mood: number): 'positive' | 'neutral' | 'negative' {
...
export function formatRecentEntries(journals: JournalEntry[], count: number = 3): DashboardEntry[] {
    return journals.slice(0, count).map(entry => ({
        ...
        sentiment: getSentimentFromMoodInt(parseInt(entry.userMood)),
    }));
}